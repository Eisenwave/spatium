...
public class VectorTest {
    ...
    public void transformScale() throws Exception {
        Vector vector = Vector.fromXYZ(1, 2, 3);
        Matrix transform = Matrix.fromScale(3, 2, 1);

        assertEquals(Vector.fromXYZ(3, 4, 3), vector.transform(transform));
    }

    private final static Vector
            POS_X = Vector.fromXYZ(1, 0, 0),
            POS_Y = Vector.fromXYZ(0, 1, 0),
            POS_Z = Vector.fromXYZ(0, 0, 1),
            NEG_X = Vector.fromXYZ(-1, 0, 0),
            NEG_Y = Vector.fromXYZ(0, -1, 0),
            NEG_Z = Vector.fromXYZ(0, 0, -1);
